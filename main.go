...
func usernameHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.URL.Query().Has("page") {
		...
		if err != nil {
			...
			fmt.Fprintf(w, "{\"message\": \"page '%s' is not a number\"}", pageInput)
			return
		}
		page = pageNumber
	}
	...
	if err != nil {
		if e, ok := errors.AsType[githubgist.UserNotFoundErr](err); ok {
			...
			w.Header().Add("Content-Type", "application/json")
			fmt.Fprintf(w, "{\"message\": \"username '%s' does not exist\"}", username)
		} else {
...
func reportUnexpectedError(err error, w http.ResponseWriter, username string) {
	...
	w.Header().Add("Content-Type", "application/json")
	fmt.Fprintf(w, "{\"message\": \"an error occurred while getting the gists of the username '%s'. please contact support or the api admin to resolve the issue\"}", username)
}
